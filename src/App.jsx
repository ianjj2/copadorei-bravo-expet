...
function App() {
  ...
  const loadParticipants = async (page = 1) => {
    try {
      setIsLoading(true);
      const data = await participants.getAll(page, PARTICIPANTS_PER_PAGE);
      setParticipantsList(prev => page === 1 ? data : [...prev, ...data]);
      setHasMore(data.length === PARTICIPANTS_PER_PAGE);
      setCurrentPage(page);
    } catch (error) {
